...
export default class Notifications extends React.Component {
    ...
    async componentDidMount() {
        ...
        AsyncStorage.getItem('unverifieduserloggedin').then((unverifieduserloggedin) => {
            if (unverifieduserloggedin) {
                this.setState({ unverifieduserloggedin: unverifieduserloggedin });
                console.log(this.state.unverifieduserloggedin);
            }
        });

        AsyncStorage.getItem('verifieduserloggedin').then((verifieduserloggedin) => {
            if (verifieduserloggedin) {
                this.setState({ verifieduserloggedin: verifieduserloggedin });
                console.log(this.state.verifieduserloggedin);
            }
        });
    ...
    getnotifications = () => {
        ...
        fetch("https://romegamart.com/api/my-notifications/" + this.state.userid, requestOptions)
            ...
            .then(result => {
                ...
                for (let i = 0; i < result.data.length; i++) {
                    ...
                    this.state.listofnotifications.push(result.data[i])
                }
    ...
    rendernotifications = () => {
        return this.state.listofnotifications.map((item) => {
            return (
                <View style={{ flexDirection: 'row', borderBottomColor: '#DDDDDD', borderBottomWidth: 3, height: Dimensions.get('window').height / 8, }}>
                    {/* source={require('../../assets/39.png')} */}
                    <Image key={item.id} source={{ uri: item.image }} style={{ height: Dimensions.get('window').height / 9, width: Dimensions.get('window').width / 5, marginRight: 10 }} />
    ...
    checkuserlogin = () => {
        if (this.state.unverifieduserloggedin === 'unverifieduserloggedin') {
            this.props.navigation.navigate('Unverifiedhome')
        }
        else if (this.state.verifieduserloggedin === 'verifieduserloggedin') {
                        ...
                        {this.state.showemptylist ?
                            <View>
                                <View style={{ width: Dimensions.get('window').width, alignItems: 'center', justifyContent: 'center', marginTop: 30 }}>
                                    <Image
                                        source={{ uri: 'https://romegamart.com/third-party/bell.png' }}
                                        // source={require('../../assets/bell.png')} 
                                        style={{ height: Dimensions.get('window').height / 7, width: Dimensions.get('window').width / 3, marginBottom: 20 }} />
                                    <Text style={{ fontSize: 34, marginBottom: 15 }}>
                            ...
                            :
                            <View>
                                <View style={{ width: Dimensions.get('window').width, alignItems: 'center', justifyContent: 'center', marginTop: 15, marginBottom: 15, borderBottomColor: '#DDDDDD', borderBottomWidth: 8 }}>
                                    <Image
                                        source={{ uri: 'https://romegamart.com/third-party/bell.png' }}
                                        //source={require('../../assets/bell.png')} 
                                        style={{ height: Dimensions.get('window').height / 7, width: Dimensions.get('window').width / 3, marginBottom: 20 }} />
                   ...
                   {this.rendernotifications()} */}
                                ...
                                <View>
                                    <View style={{ marginTop: 20, alignItems: 'center', justifyContent: 'center', }}>
                                        <TouchableOpacity
                                            style={{ height: Dimensions.get('window').height / 15, width: Dimensions.get('window').width - 50, backgroundColor: '#2e3191', borderRadius: 8, alignItems: 'center', justifyContent: 'center' }}
                                            onPress={() => this.props.navigation.navigate('Verifiedhome')}
                                        >
