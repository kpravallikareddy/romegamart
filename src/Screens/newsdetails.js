...
import { View, BackHandler, Modal, ActivityIndicator, Text, TextInput, CheckBox, Button, Image, ScrollView, Dimensions, Alert, StyleSheet, StatusBar, SafeAreaView, TouchableOpacity, ImageBackground } from 'react-native';
...
export default class Newsdetails extends React.Component {
    constructor(props) {
        ...
        this.state = {
            showloader: false,
            newsid: this.props.route.params.id,
            listofnewsdetails: [],
            listofnewsdetailsimage: [],
            listofnewsdetailsdescription: [],
            userid: '',
            showmenu: false,
        }
    ...
    getnewsdetails = () => {
        ...
        fetch(BASE_URL + "news-details/" + this.state.newsid, requestOptions)
            .then(response => response.json())
            .then(result => {
                //  console.log(result)
                for (let i = 0; i < result.data.length; i++) {
                    this.state.listofnewsdetails.push(result.data[i])
                    this.state.listofnewsdetailsimage.push(result.data[i].image)
                    this.state.listofnewsdetailsdescription.push(result.data[i].description)
                }
                this.setState({ showloader: !this.state.showloader })
                console.log('newsimage', this.state.listofnewsdetails[0].image)
            })
            .catch(error => console.log('error', error));
    }

    rendernewsdetails = () => {
        return this.state.listofnewsdetails.map((item) => {
            return (
                <View>
                    <Image key={item.id} source={{ uri: item.image }} style={{ width: Dimensions.get('window').width, height: 200 }} />
    ...
    renderdetails = () => {
        return (
            <View>
                <Image source={this.state.listofnewsdetailsimage[0]} style={{ width: Dimensions.get('window').width, height: 200 }} />
                <Text style={styles.textintips}>
                    {this.state.listofnewsdetailsdescription[0]}
                </Text>
            </View>
        )
    }
                                    ...
                                    <View style={{ width: Dimensions.get('window').width / 2 + 50, backgroundColor: '#ffffff', borderRightColor: '#000000', borderRightWidth: 2, height: Dimensions.get('window').height - 160 }}>
                                        ...
                                        >
                                            <View style={{ marginLeft: 5 }}>
                                                ...
                                                <TouchableOpacity
                                                    style={styles.butt, [{ borderBottomColor: '#DDDDDD', borderBottomWidth: 2, marginLeft: 5,marginRight:5,marginRight:5 }]}
   ...
   source={require('../../assets/comlogo.jpeg')}
/>
                </View>
